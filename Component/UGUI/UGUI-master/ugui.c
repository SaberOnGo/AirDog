...
void UG_FONT_StructInit(UG_FONT * f, 
    unsigned char* p,
   FONT_TYPE font_type,
   UG_S16 char_width,
   UG_S16 char_height,
...
void UFONT_Init(void)
{
        ...
        UG_FONT_StructInit(&FONT_6X8,       (unsigned char*)font_6x8,       FONT_TYPE_1BPP,6,8,0,255,NULL);
        UG_FONT_StructInit(&FONT_8X12,    (unsigned char*)font_8x12,    FONT_TYPE_1BPP,8,12,0,255,NULL);
        UG_FONT_StructInit(&FONT_10X16, (unsigned char*)font_10x16, FONT_TYPE_1BPP,10,16,0,255,NULL);
        UG_FONT_StructInit(&FONT_12X20, ( unsigned char*)font_12x20,FONT_TYPE_1BPP,12,20,0,255,NULL);
        UG_FONT_StructInit(&FONT_16X26, ( unsigned char*)font_16x26,FONT_TYPE_1BPP,16,26,0,255,NULL);
        UG_FONT_StructInit(&FONT_24X40, (unsigned char*)font_24x40,FONT_TYPE_1BPP,24,40,0,255,NULL);
}
...
/*
// ²ÎÊý: x: ×Ö·ûÎ»ÖÃ, ²»ÊÇÏñËØµÄÎ»ÖÃ. Èç 320(¿í)x240(¸ß)µÄÆÁÄ», 
                      Èç×Ö·ûÎª 32 pixel(¿í)x 24 pixel(¸ß), Ôò×Ö·ûµÄÎ»ÖÃÎª 0 - 9
                  y: ×Ö·ûÎ»ÖÃË÷Òý, ·ÇÏñËØÎ»ÖÃ
*/
...
void _UG_PutChar( char chr, UG_S16 x, UG_S16 y, UG_COLOR fc, UG_COLOR bc, const UG_FONT* font)
{
   ...
   if ( gui->driver[DRIVER_FILL_AREA].state & DRIVER_ENABLED )
   {
      ...
      push_pixel = (void(*)(UG_COLOR))(((DrvFillArea_FuncType)(gui->driver[DRIVER_FILL_AREA].driver))(x,y,x+actual_char_width-1,y+font->char_height-1));
      ...
      if (font->font_type == FONT_TYPE_1BPP)
	  {
		  ...
		  for( j=0;j<font->char_height;j++ )
		  {
			 ...
			 for( i=0;i<bn;i++ )
			 {
				...
				b = font->p[index++];
				#else
				b = UGUI_FONT_GetPixel((FONT_DESC *)font->p, index++);
				#endif
				for( k=0;(k<8) && c;k++ )
				{
				   if( b & 0x01 )
