...
u16 screen_width  = X_MAX_PIXEL,
    screen_height = Y_MAX_PIXEL;
...
static void LCD_pinsInit(void) {
   ...
   // RCC_PCLK2Config(RCC_HCLK_Div2);
    ...
    GPIO_Init(TFT_CS_PORT, &gpioStructure);

    gpioStructure.GPIO_Pin  = TFT_DC_PIN;
    GPIO_Init(TFT_DC_PORT, &gpioStructure);

    gpioStructure.GPIO_Pin  = TFT_RESET_PIN;
    GPIO_Init(TFT_RESET_PORT, &gpioStructure);
...
static void LCD_configure(void) {
    ...
    while (1) {
      ...
      TFT_CS_SET;
        ...
        #if 1
      #if USE_HORIZONTAL
      LCD_setOrientation(ORIENTATION_HORIZONTAL_MIRROR);  
      
	#else
	LCD_setOrientation(ORIENTATION_VERTICAL);  
	#endif
	#endif
