...
//inline 
static void LCD_drawChar(u16 x0, u16 y0, uint8_t c, u16 color, u16 bg, uint8_t size) {
   ...
   // if (x0 >= LCD_getWidth() || y0 >= LCD_getHeight() || x1 < 0 || y1 < 0) return;
    if(x0 >= LCD_getWidth() || y0 >= LCD_getHeight()) return;
    ...
    for (i = 0; i < 6; i++) {
        ...
        for (j = 0; j < 8; j++, line >>= 1) {
            ...
            for (sx = 0; sx < size; ++sx)
			{
                for (sy = 0; sy < size; ++sy) 
				{
					cnt = mx + my + sy * scaledWidth + sx;
					if(cnt >= sizeof(charPixels))
					{
					   os_printf("pixel cnt = %d\n", cnt);
					   while(1);
					   //return;
					}
                    charPixels[cnt] = pixelColor;
                }
            }
        }
    }
